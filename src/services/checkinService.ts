...
export const matchesCustomerLookup = (
  customer: { businessId?: string; storeId?: string; primaryStoreId?: string; phone?: string },
  input: { businessId: string; storeId: string; phone: string }
): boolean => {
...
export const checkinService = {
  lookupCustomer: async (input: { businessId: string; storeId: string; phone: string }) => {
    ...
    const customer = customers.find((candidate) => matchesCustomerLookup(candidate as any, input));
  ...
  getStoresByBusiness: async (input: { businessId: string; storeId: string }) => {
    if (!mongoose.connection.db) {
      throw new Error('Database is not connected');
    }

    const storesCollection = mongoose.connection.db.collection<any>('stores');
    const stores = await storesCollection
      .find({ businessId: input.businessId })
      .sort({ name: 1, storeId: 1 })
      .toArray();

    const normalizedMap = new Map<string, { storeId: string; name: string }>();
  ...
  getOwnersByBusiness: async (input: { businessId: string; storeId: string }) => {
    if (!mongoose.connection.db) {
      throw new Error('Database is not connected');
    }

    const storesCollection = mongoose.connection.db.collection<any>('stores');
    const ownersCollection = mongoose.connection.db.collection<any>('owners');
    ...
    const ownerList = new Map<string, { id: string; firstName: string; lastName: string }>();
  ...
  getOwnerReport: async (input: {
    ...
    type TreeNode = {
      id: string;
      nodeType: 'store' | 'today' | 'month' | 'week' | 'day' | 'technician' | 'customer' | 'service';
      label: string;
      subtotal: number;
      children: TreeNode[];
    };
    ...
    const roots: TreeNode[] = targetStoreIds.map((storeId) => ({
      id: `store-${storeId}`,
      nodeType: 'store',
    ...
    const rootByStoreId = new Map<string, TreeNode>();
